...
def fit_ev_logistic(df,
    ...
    X_ori = X.copy()
    # if endpoint=='Lactate':
    X_ori['INTER_SEX_FFP'] = X_ori['SEX']*X_ori['TF_FFP']
    ...
    must_inclusive_cols = ['INTER_SEX_FFP']
    must_drop_cols = []
...
for endpoint in ['PLT','Hb','WBC','ANC', 'Lactate']:
    ...
    if not os.path.exists(f'{output_dir}/b1da/mvlreg_output(forced3)'):
        os.mkdir(f'{output_dir}/b1da/mvlreg_output(forced3)')

    out_csv = f"{output_dir}/b1da/mvlreg_output(forced3)/b1da_lnz_mvlreg_{endpoint}_res.csv"
    ...
    # for age_subgroup in ['Elderly','Total_Adult']:
        ...
        if not os.path.exists(f'{output_dir}/b1da/mvlreg_output(forced3)/datasubset'):
            os.mkdir(f'{output_dir}/b1da/mvlreg_output(forced3)/datasubset')
        df.to_csv(f"{output_dir}/b1da/mvlreg_output(forced3)/datasubset/b1da_lnz_mvlreg_datasubset({age_subgroup})({endpoint}).csv", index=False, encoding='utf-8-sig')
...
multivar_totres_df.to_csv(f"{output_dir}/b1da/mvlreg_output(forced3)/b1da_lnz_mvlreg_total_results.csv", index=False, encoding='utf-8-sig')
...
# raise ValueError
covar_info_df_saving.to_excel(f"{output_dir}/b1da/mvlreg_output(forced3)/b1da_lnz_mvlreg_covar_info.xlsx", index=False, encoding='utf-8-sig')
...
uni_res_df.to_csv(f"{output_dir}/b1da/mvlreg_output(forced3)/b1da_lnz_mvlreg_univar_res_table.csv", index=False, encoding='utf-8-sig')
multi_res_df.to_csv(f"{output_dir}/b1da/mvlreg_output(forced3)/b1da_lnz_mvlreg_multivar_res_table.csv", index=False, encoding='utf-8-sig')
...
tot_res_df.to_csv(f"{output_dir}/b1da/mvlreg_output(forced3)/b1da_lnz_mvlreg_total_res_table.csv", index=False, encoding='utf-8-sig')
