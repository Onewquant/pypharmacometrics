...
comed_files = glob.glob(f"{resource_dir}/comed_frag_data/lnz_cdw_comed(*).csv")
comed_res_df = list()
for comed_fpath in comed_files: #break

    # drug_res_df = list()
    drugname = comed_fpath.split('lnz_cdw_comed(')[-1].split('_')[0]
    adr_name = comed_fpath.split('_')[-1].split(').csv')[0]

    raw_df = pd.read_csv(comed_fpath, encoding='utf-8-sig')
    # raw_df.columns
    ...
    alter_acting_dict = {
        ...
        '1일 12회': ('q2.4h',
                   '02:00/Y, 04:00/Y, 05:00/Y, 08:00/Y, 10:00/Y, 11:00/Y, 14:00/Y, 16:00/Y, 17:00/Y, 19:00/Y, 21:00/Y, 23:00/Y'),
    ...
    sdf = pd.concat([df1, df2]).sort_values(['UID', 'DATE', 'ACTING'])[
        ['UID', 'DATE', 'DOSE', 'ROUTE', 'INTERVAL', 'ADDL', 'ACTING', 'PLACE']].copy()
    ...
    for uid, uid_sdf in sdf.groupby('UID'):  # break
        ...
        for sdf_inx, uid_sdf_row in uid_sdf.iterrows():  # break
            ...
            uid_sdf_dates = uid_sdf_dates.union(set(
                dates_every_step_days(start_date=uid_sdf_row['DATE'], day_step=uid_sdf_row['II_APPLIED'],
                                      addl=uid_sdf_row['ADDL'], fmt="%Y-%m-%d", include_start=True)))

        uid_dates_rows = pd.DataFrame(uid_sdf_dates, columns=['DATE']).sort_values(['DATE'], ignore_index=True)
        uid_dates_row = {'UID': uid, 'ADRLIST':adr_name, 'DRUG': drugname, 'DATE_LIST': tuple(uid_dates_rows['DATE'])}
...
comed_res_df.to_csv(f"{output_dir}/lnz_final_comed_df.csv", encoding='utf-8-sig', index=False)
