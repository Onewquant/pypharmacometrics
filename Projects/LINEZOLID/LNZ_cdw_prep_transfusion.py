...
# raw_df['DATE'] = raw_df['DATE'].map(lambda x:'T'+x.split(' ')[-1])
raw_df['TF_TYPE'] = raw_df['TF_TYPE'].map(lambda x:x.split('Apheresis ')[-1].split('Washed ')[-1].split(' ')[0]).map({'FFP':'TF_FFP', 'RBC':'TF_RBC', 'Red':'TF_RBC', 'PLT':'TF_PLT', 'Cryoprecipitate':'TF_CRYO','Whole':'TF_WBLD','지정혈액1':'TF_WBLD','자가혈액1':'TF_WBLD', '자가혈액2':'TF_WBLD', 'Plateletpheresis':'TF_PLT'})
raw_df = raw_df[['UID','DATE','TF_TYPE','TF_COUNT']].copy()

if not os.path.exists(f'{output_dir}'):
    os.mkdir(f'{output_dir}')
raw_df.to_csv(f"{output_dir}/lnz_final_transfusion_df.csv", encoding='utf-8-sig', index=False)