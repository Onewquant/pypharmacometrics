...
result_cols = ['ID','NAME','DATETIME','DRUG','DOSE','ACTING','PERIOD','PLACE']
...
for finx, fpath in enumerate(order_files): #break
    ...
    dose_df = fdf[fdf['처방지시'].map(lambda x: ('amikacin' in x.lower()) and ('농도' not in x.lower()) and ('TDM (자문)' not in x.lower()))].copy()
    # dose_df.iloc[0]['처방지시']
    # dose_df.iloc[1]['처방지시']
    # dose_df.iloc[2]
