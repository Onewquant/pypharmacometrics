...
########## NCA ###########
sim_df = pd.read_csv(f"{resource_dir}/sim056",encoding='utf-8', skiprows=1, sep=r"\s+", engine='python')
...
AUC_F = auc_24bf_df_1st_rows[['ID','cumAUC_ALL','cumAUC_LAST24']]

# ID별 마지막 CL, V1 뽑기
last_total_auc_df = total_auc_df.drop_duplicates(['ID'], keep='last')
AUC_F = AUC_F.merge(last_total_auc_df[['ID','CL','V1']].copy(), how='left',on=['ID'])

# ########## 24H 중에 maximum (Cmax,Ctrough) 구하기 (Dosing 데이터 기준) ##########
#
# sim_24bf_df = sim_df.merge(add_on_df, on='ID', how='left')
# sim_24bf_df = sim_24bf_df[sim_24bf_df['TIME'] >= sim_24bf_df['TIME24BF']].copy()
# sim_24bf_df
