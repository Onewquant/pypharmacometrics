...
# df['MIN_DATE'] = df['TDM_DATE'].copy()
df['MAX_DATE'] = df['TDM_DATE'].copy()
df = df.groupby(['EMR ID'], as_index=False).agg({'name':'first','DEP':'first','TDM_DATE':'min','MAX_DATE':'max'})
df['TDM_DATE'] = df['TDM_DATE'].map(lambda x:x.split(' ')[0])
df['MAX_DATE'] = df['MAX_DATE'].map(lambda x:x.split(' ')[0])
## ORDER 수집

existing_files = glob.glob(f"{prj_dir}/order/{drug_name}_order(*).xlsx")
existing_id_list = [int(f.split(f'{drug_name}_order(')[-1].split('_')[0]) for f in existing_files]
...
for inx, row in df.iterrows(): #break
    ...
    if row['EMR ID'] in [10008621]: # 오더조회에는 기록 있는데 전체오더조회에는 안 나옴
        continue
...
#         break
    ...
    # pid_lab_df = pd.read_excel(f"{prj_dir}/lab/{drug_name}_lab({pid}_{row['name']}).xlsx")
    ...
    min_date = (datetime.strptime(row['TDM_DATE'], '%Y-%m-%d') - timedelta(days=30)).strftime('%Y-%m-%d')
    # raise ValueError

    # today_date = datetime.today().strftime('%Y-%m-%d')
    today_date = (datetime.strptime(row['TDM_DATE'], '%Y-%m-%d') + timedelta(days=60)).strftime('%Y-%m-%d')

    # 수집된 Order_frag 데이터의 마지막날짜 가져오기
    # raise ValueError
    existing_order_frag_files = glob.glob(f"{prj_dir}/order_frag/{pid}_{row['name']}/{drug_name}_order({pid}_{row['name']}_*).xlsx")
    ...
    for date_inx, date_row in intervals_df.iterrows():
        ...
        while raw_order in ['', waiting_message1, waiting_message2, min_date, today_date]:
            ...
            if len(test_raw_order.split('\n'))<=2:
                if row['name'] in []:
                    pass
                # if row['name'] in ['이학준','김동규', '김지원','안동관','오민권', '박종웅','정다워','장세호','여범석', '하승효','이원요', '이은채', '김옥순', '한재웅', '장선이','이규환','김민이','이종민','이상존','박정훈','장현정','정준현','한상희']:
                #     pass
                else:
                    continue
            # else:
        ...
        print(f"({inx}-{date_inx}) {pid} / {row['name']} / {row['DEP']} / {len(order_frag_df)} rows / start: {date_row['start_date']} / end: {date_row['end_date']}")

        if not os.path.exists(f"{prj_dir}/order_frag/{pid}_{row['name']}"):
            os.makedirs(f"{prj_dir}/order_frag/{pid}_{row['name']}")
        order_frag_df.to_excel(f"{prj_dir}/order_frag/{pid}_{row['name']}/{drug_name}_order({pid}_{row['name']}_{date_inx}).xlsx",index=False)
        time.sleep(1)
    ...
    order_frag_files = glob.glob(f"{prj_dir}/order_frag/{pid}_{row['name']}/{drug_name}_order({pid}_{row['name']}_*).xlsx")
    order_df = list()
    ...
    if not os.path.exists(f"{prj_dir}/order"):
        os.makedirs(f"{prj_dir}/order")
    order_df.to_excel(f"{prj_dir}/order/{drug_name}_order({pid}_{row['name']}).xlsx", index=False)
