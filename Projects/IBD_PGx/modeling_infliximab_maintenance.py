...
for inx, row in ind_cons_df.iterrows():
    ...
    end_date = (datetime.strptime(start_date,'%Y-%m-%d') + timedelta(days=98)).strftime('%Y-%m-%d')
    ...
    if len(unique_date_df)>0:
        ...
        for dup_date_inx, dup_date_row in unique_date_df.iterrows():
            ...
            if len(dupmdv_df_frag)>0:
                ...
                ind_df_frag_list.append(pd.concat([nodup_df_frag, dupdv_df_frag]))
...
inf_ind_df.to_csv(f'{output_dir}/infliximab_maintenance_datacheck.csv',index=False, encoding='utf-8-sig')
ada_ind_df.to_csv(f'{output_dir}/adalimumab_maintenance_datacheck.csv',index=False, encoding='utf-8-sig')
...
inf_ind_df.to_csv(f'{output_dir}/infliximab_maintenance_df.csv',index=False, encoding='utf-8-sig')
ada_ind_df.to_csv(f'{output_dir}/adalimumab_maintenance_df.csv',index=False, encoding='utf-8-sig')
