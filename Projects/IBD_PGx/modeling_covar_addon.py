...
totlab_df = totlab_df.rename(columns={'Albumin': 'ALB', 'Calprotectin (Stool)': 'CALPRTSTL', 'Calprotectin (Serum)': 'CALPRTSER'})
...
for mode_str in ['induction','maintenance','integrated']:
    modeling_df = pd.read_csv(f'{output_dir}/infliximab_{mode_str}_datacheck.csv')
    ...
    modeling_df.to_csv(f'{output_dir}/infliximab_{mode_str}_df.csv',index=False, encoding='utf-8-sig')
