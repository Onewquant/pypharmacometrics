...
- 표·그림 번호: Table 1–5, Figure 1–3, Supplementary Table S1–S5,
  Supplementary Figure S1–S3 (`core_fig_tab/CAPTIONS.md` 참조; S1 GOF, S2 VPC는
  NONMEM run 89에서 내보낸 PDF, S3는 rs396991 유전형별 CL 산점도).
  2026-09-07: rs1061622 전용 자료(Results 문단, Suppl S6, Suppl Fig S4,
  S3 민감도 행) 삭제 — 비유의 변이를 따로 다룰 근거 없음
...
without infliximab administration records (n = 41) and one patient whose
data started in the maintenance phase with only a single concentration
measurement, which was used to initialize the model and could not inform
individual parameter estimation, were excluded (n = 42 in total); the
infliximab PopPK modeling cohort therefore comprised 97 patients.
...
within each analysis period. The induction-phase window extended from the
first infliximab dose to the third dose, and the maintenance-phase window
from the start of maintenance dosing to the last dose administered on or
before the one-year assessment (10–12 months after the start of
maintenance dosing), or to the end of follow-up when no one-year
assessment was available; the overall treatment period combined both
windows. Time-varying covariates, including ADA status, were taken from
the records within each window, so ADA positivity in the pharmacogenomic
...
*(기존 docx 문단 유지 — Table 1, Table 2, Figure 2. 2026-09-07: Model
 Evaluation 문단에 "(Supplementary Figures S1 and S2)" 참조 추가, Methods
 Model Evaluation에 VPC 설정(200 simulations, 8 bins by count) 한 문장 추가.
 최종모델(run 89)은
...
maintenance window and were therefore classified as ADA-negative in these
analyses (Table 1 reports ADA positivity at any time during follow-up).
...
*(Discussion은 다른 저자 담당 — 아래 rs1061622 문장은 참고용 제안)*
...
within the analysis periods, with none in the induction-phase window; the
analysis was therefore able
...
4. ~~VPC/GOF 플롯 추가 여부~~ → **추가 완료 (2026-09-07)**: Suppl Figure S1
   (GOF), S2 (VPC, run/vpc_89). 단, VPC PDF의 축 제목이 xpose 기본값
   ("Independent variable"/"Dependent variable")이라 재출력 권장
