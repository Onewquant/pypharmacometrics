...
for finx, fpath in enumerate(order_files): #break
    ...
    dose_df = fdf[fdf['처방지시'].map(lambda x: (('prednisone' in x.lower()) or ('prednisolone' in x.lower()) or ('hydrocortison' in x.lower()) or ('budesonide' in x.lower())) and ('quantification' not in x.lower()))].copy()
    # dose_df.iloc[0]['처방지시']
    # dose_df.iloc[1]['처방지시']
    # dose_df.iloc[2]
    # dose_df['처방지시'].unique()
    ...
    if len(dose_df)==0:
        print(f"({finx}) {pname} / {pid} / No administration records (Steroid)")
        continue
    else:
    ...
    regex_pattern = r'\(prednisone|\(prednisolone|\(hydrocortison|\(budesonide'
    ...
    # dose_df['처방지시'].iloc[0]
    dose_df['ROUTE'] = dose_df['처방지시'].map(lambda x: x.split('[')[-1].split(']')[0] if (" [P.O] " in x) or ("[per Rectal]" in x) or ("[Apply]" in x) or ("[SC]" in x) or ("[IVS]" in x) or ("[IV]" in x) or ("[IM]" in x) or ("[MIV]" in x) else 'ETC')
    # dose_df['처방지시'].iloc[0]
    ...
    for dose_inx, dose_row in dose_df.iterrows():
        ...
        if (" [P.O] " in x) or ("[per Rectal]" in x) or ("[SC]" in x) or ("[IVS]" in x) or ("[IV]" in x) or ("[IM]" in x) or ("[MIV]" in x):
            # raise ValueError
            try: dose_val = float(re.findall(r'\d+[\.]?\d*mg',x)[0].replace('mg','').strip())
            except:
                if ("Levan H oint" in x):
                    dose_val = int(x.split('[S] 총 ')[-1].split('tub')[0].strip()) * 10
            ...
            if ("[Drop]" in x) or ("[Apply]" in x) or ("[Inhalation]" in x):
                dose_series.append(np.nan)
                continue
            print('PO 아님')
            raise ValueError
    ...
    dose_df = dose_df.dropna(subset=['DOSE'])
...
# dose_result_df['DATETIME'] = dose_result_df['DATETIME'].replace('2016-04-22T19:','2016-04-22T19:00').replace('2020-06-13T15:','2020-06-13T15:00')
...
for inx, row in addl_dose_result_df[addl_dose_result_df['ADDL']>0].iterrows():
    ...
    for addl_num in range(0,max_addl):
        ...
        row['PERIOD'] = 'x1'
        row['ADDL']=0
        row['DATETIME'] = (datetime.strptime(init_datetime.split(':')[0],'%Y-%m-%dT%H')+timedelta(addl_num*row['II']/24)).strftime('%Y-%m-%dT%H:%M')
        if addl_num==0:
            continue
        else:
    ...
    if len(addl_frag_df)!=0:
        print(f'{row["ID"]} / {row["DATETIME"]} / {row["DRUG"]} / ADDL ({max_addl})')
        addl_added_df.append(pd.concat(addl_frag_df))
    else:
        print(f'{row["ID"]} / {row["DATETIME"]} / {row["DRUG"]} / No ADDL')
...
addl_dose_result_df.to_csv(f"{output_dir}/cm_df(STEROID).csv", encoding='utf-8-sig', index=False)
