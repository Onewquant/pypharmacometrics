...
for finx, fpath in enumerate(order_files): #break
    ...
    dose_df = fdf[fdf['처방지시'].map(lambda x: (('azathioprine' in x.lower()) or ('mercaptopurine' in x.lower()) or ('methotrexate' in x.lower()) or ('cyclosporine' in x.lower())) and ('quantification' not in x.lower()))].copy()
    # dose_df.iloc[0]['처방지시']
    # dose_df.iloc[1]['처방지시']
    # dose_df.iloc[2]
    # dose_df['처방지시'].unique()
    ...
    if len(dose_df)==0:
        print(f"({finx}) {pname} / {pid} / No administration records (Immunomodulator)")
        continue
    else:
    ...
    regex_pattern = r'\(azathioprine|\(mercaptopurine|\(methotrexate|\(cyclosporine'
    ...
    # dose_df['처방지시'].iloc[0]
    dose_df['ROUTE'] = dose_df['처방지시'].map(lambda x: x.split('[')[-1].split(']')[0] if (" [P.O] " in x) or ("[per Rectal]" in x) or ("[Apply]" in x) or ("[SC]" in x) else 'ETC')
    # dose_df['처방지시'].iloc[0]
    ...
    for dose_inx, dose_row in dose_df.iterrows():
        ...
        if (" [P.O] " in x) or ("[per Rectal]" in x) or ("[Apply]" in x) or ("[SC]" in x):
            # raise ValueError
...
addl_dose_result_df.to_csv(f"{output_dir}/cm_df(IM).csv", encoding='utf-8-sig', index=False)
