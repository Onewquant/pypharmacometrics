...
from resource.prepare_directories import raw_df

# result_type = 'R'

prj_name = 'Valproic acid'
prj_dir = f'C:/Users/tldus/PycharmProjects/DILI/resource'
resource_dir = f'{prj_dir}/prep'
output_dir = f'C:/Users/tldus/PycharmProjects/DILI/resource/prep/drug'
if not os.path.exists(output_dir):
    os.mkdir(output_dir)

# 암호 걸린 파일 열기
file_path = f"{resource_dir}/DILI_Valproic acid_DRUG.xlsx"
password = "snubhsnubh"
...
# pandas로 읽기
# raw_df = pd.read_excel(f"{resource_dir}/DILI_Valproic acid_DRUG.xlsx",engine="openpyxl")
...
# 수행시간 -
# 리네졸리드: 10:22/Y, 21:55/Y
# VPA: Y(2015-10-02 06:34:00), Y(2015-10-02 12:55:00), Y(2015-10-02 20:57:00)
# raw_df['약품명(일반명)'] = raw_df['DRUG'].copy()
raw_df = raw_df.rename(columns={'환자번호':'UID','수행코드(수행시간)':'ACTING','약품 오더일자':'DATE', '[실처방] 용법':'REGIMEN','[실처방] 처방일수':'DAYS', '[함량단위환산] 1일 처방량':'DAILY_DOSE','[실처방] 1회 처방량':'DOSE', '[실처방] 투약위치':'PLACE',"[실처방] 경로":'ROUTE','약품명(성분명)':'DRUG','[실처방] 처방비고':'ETC_INFO'})
# raw_df['약품명(일반명)'] = raw_df['DRUG']
raw_df = raw_df[~raw_df['DRUG'].isna()]
# raw_df = raw_df[raw_df['약품명(일반명)']!='Valeptol SR Tab. (환인제약)']
raw_df['DOSE'] = raw_df['DOSE'].astype(str)
raw_df['DOSE'] = raw_df.apply(lambda x:float(re.findall(r'\d+[\.]?\d*',x['DOSE'])[0]) if ('mg' in x['DOSE']) else (float(re.findall(r'\d+[\.]?\d*',x['DOSE'])[0]) * float(re.findall(r'(\d*\.?\d+)',x['DRUG'])[0])), axis=1)
raw_df['UID'] = raw_df['UID'].astype(str)
drugname = 'Valproic acid'
raw_df['DRUG'] = 'Valproic acid'
raw_df.dtypes
df = raw_df.copy()
df = df[~df['REGIMEN'].isna()]
...
for inx, row in df.iterrows():
    ...
    # if row['']
    seq_dose_patterns = re.findall(r'\d+\.*\d*', str(row['DOSE']))
...
# df[df['REGIMEN'].isna()]['ETC_INFO'].unique()
# df.iloc[44970]
# df.iloc[49241]
# df.iloc[49245]
...
df['DAYS'].unique()
df['ADDL'] = df['DAYS']
df['ADDL'] = df['DAYS'].map(lambda x:int(x.replace(' 일','')))
...
for inx, row in df.iterrows():
    ...
    for actval in rep_acting_str.split(','): #break
        ...
        ## Y/날짜 시간일때 Y\((\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})\)

        y_val = re.findall( r"Y\(\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d\)",actval)
        if len(y_val) > 0:
            new_actval_str+='_' + y_val[0].replace("Y(","").replace(" ","T").replace(")","")
            continue
        else:
            pass
        ## P/날짜 시간일떄(집에서 자가투약)
         y_val = re.findall(r"P\(\d\d\d\d-\d\d-\d\d \d\d:\d\d\)", actval)
        if len(y_val) > 0:
            new_actval_str += '_' + y_val[0].replace("P(", "").replace(" ", "T").replace(")","")
            continue
        else:
            pass

         ## S/날짜 시간일때(자가투약)
        y_val = re.findall(r"S\(\d\d\d\d-\d\d-\d\d \d\d:\d\d\)", actval)
        if len(y_val) > 0:
            new_actval_str += '_' + y_val[0].replace("S(", "").replace(" ", "T").replace(")","")
            continue
        else:
             pass
...
df.to_csv(f"{output_dir}/{drugname}_dt_dose_df.csv", encoding='utf-8-sig', index=False)
...
final_dose_df.to_csv(f"{output_dir}/VPA_dose_df.csv", encoding='utf-8-sig', index=False)
